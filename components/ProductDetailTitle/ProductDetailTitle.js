...
class ProductDetailTitle extends Component {
  ...
  render() {
  	...
  	return (
  		<Grid item sm={12}>
  			{title && <Typography style={{color:"#fff",fontSize:"30px"}} gutterBottom={true} variant={variant}>{title}</Typography>}
  			{pageTitle && <Typography style={{color:"#fff"}} component="h2" variant="h6">{pageTitle}</Typography>}
  		</Grid>
  	);
  }
}
